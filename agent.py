...
MINUTES_WINDOW   = 20  # look back this many minutes (matches scheduler interval)
...
async def fetch_onefootball_article(url: str) -> tuple[str, str | None, datetime.datetime | None]:
    """Fetch body text + og:image + publish date from an individual OneFootball article."""
    try:
        async with async_playwright() as pw:
            browser = await pw.chromium.launch(
                headless=True,
        ...
        # Try to extract publish date from <time> or meta tags
        ...
        if time_tag and time_tag.get("datetime"):  # type: ignore[union-attr]
            try:
                pub_dt = datetime.datetime.fromisoformat(
                    str(time_tag["datetime"]).replace("Z", "+00:00")  # type: ignore[index]
                )
            except ValueError:
                pass
        if not pub_dt:
            meta_date = soup.find("meta", property="article:published_time")
            if meta_date and meta_date.get("content"):  # type: ignore[union-attr]
                try:
                    pub_dt = datetime.datetime.fromisoformat(
                        str(meta_date["content"]).replace("Z", "+00:00")  # type: ignore[index]
                    )
                except ValueError:
                    pass
...
async def run_pipeline() -> None:
    ...
    for article in new_of:
        body, better_img, pub_dt = await fetch_onefootball_article(article["url"])

        # Skip if article is older than MINUTES_WINDOW (when date is available)
        if pub_dt:
            # Make cutoff_dt naive if pub_dt is naive, for safe comparison
